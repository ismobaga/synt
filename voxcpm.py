...
def _wav_to_base64(sample_rate: int, wav_np: np.ndarray) -> str:
    ...
    with io.BytesIO() as buffer:
        with wave.open(buffer, "wb") as wav_file:
            ...
            wav_file.writeframes(wav_np.tobytes())
...
def create_api_app(demo: VoxCPMDemo) -> FastAPI:
    app = FastAPI(title="VoxCPM2 Audio Generation API", version="1.0.0")
    ...
    def generate(request: GenerateRequest):
        try:
            sample_rate, wav_np = demo.generate_tts_audio(
                text_input=request.text_input,
                control_instruction=request.control_instruction,
                reference_wav_path_input=request.reference_wav_path_input,
                prompt_text=request.prompt_text,
                cfg_value_input=request.cfg_value_input,
                do_normalize=request.do_normalize,
                denoise=request.denoise,
                inference_timesteps=request.inference_timesteps,
            )
            return GenerateResponse(
                sample_rate=sample_rate,
                audio_base64=_wav_to_base64(sample_rate, wav_np),
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        choices=["gradio", "api"],
        default="gradio",
        help="Run mode: gradio UI or REST API",
    )
    parser.add_argument("--host", type=str, default="0.0.0.0", help="Host to bind")
    parser.add_argument("--port", type=int, default=8808, help="Server port")
    args = parser.parse_args()
    if args.mode == "api":
        run_api(host=args.host, port=args.port, model_id=args.model_id)
    else:
        run_demo(server_name=args.host, model_id=args.model_id, server_port=args.port)